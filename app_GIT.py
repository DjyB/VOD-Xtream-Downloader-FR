# Catalogue VOD sur Unraid - Flask + Docker + Xtream Downloader
...
# Modèle HTML (très simplifié)
TEMPLATE = """
<!DOCTYPE html>
<html>
<head>
    <title>Catalogue VOD</title>
    <style>
...
<nav>
    <a href="/">Films</a> |
    <a href="/seriale">Séries</a>
</nav>
<hr>
    {% for movie in movies %}
        <div class="item">
            <strong>{{ movie['name'] }}</strong> ({{ movie.get('rating', 'Pas de note') }})<br>
            <em>{{ movie.get('genre', 'Pas de genre') }}</em><br>
            {{ movie.get('plot', '') }}<br>
            <button onclick="download('{{ movie['stream_id'] }}', '{{ movie['name'] }}')">Télécharger</button>
        </div>
    {% endfor %}
...
def download():
    ...
    if success:
        return jsonify({"message": f"Téléchargé : {name}"})
    else:
        return jsonify({"message": f"Échec du téléchargement : {name}"}), 500
