...
def serial_detail(series_id):
    response = requests.get(f"{BASE_API}&action=get_series_info&series_id={series_id}")
    if response.status_code != 200:
        return "Erreur lors de la récupération des données de la série", 500
    info = response.json()
    serial = info['info']
...
def download_episode():
    ...
    response = requests.get(f"{BASE_API}&action=get_series_info&series_id={series_id}")
    if response.status_code != 200:
        return "Erreur lors de la récupération des métadonnées", 500
    ...
    attached = video_info.get("disposition", {}).get("attached_pic", 0)
        ...
        #return "❌ Ce n'est pas un fichier vidéo, mais une miniature (cover)", 400

    serial_name = data.get('info', {}).get('name', f"serie_{series_id}").replace('/', '_')
...
def is_episode_already_downloaded(serial_name, season, episode_num, title, ext):
    ...
    return os.path.exists(file_path) and os.path.getsize(file_path) > 1000000  # plus grand que 1 Mo
